...
export default {
  ...
  computed: {
    ...
    layoutType: {
      get ()    { return this.$store.state.Default.mainLayoutType },
      set (val) { this.$store.commit('UPDATE_MAIN_LAYOUT_TYPE', val) }
    },
    primaryColor: {
      get ()    { return this.$store.state.Default.themePrimaryColor },
    ...
    themeMode: {
      get ()    { return this.$store.state.Default.theme },
      set (val) { this.$store.dispatch('updateTheme', val) }
    },
    scrollbarTag () { return this.$store.state.Default.is_touch_device ? 'div' : 'VuePerfectScrollbar' },
    windowWidth () {
