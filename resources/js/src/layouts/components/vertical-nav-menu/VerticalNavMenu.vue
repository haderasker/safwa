...
export default {
  ...
  computed: {
    ...
    isVerticalNavMenuActive: {
      get ()    { return this.$store.state.Default.isVerticalNavMenuActive },
      set (val) { this.$store.commit('TOGGLE_IS_VERTICAL_NAV_MENU_ACTIVE', val) }
    },
    layoutType () { return this.$store.state.Default.mainLayoutType },
    reduceButton: {
      get ()    { return this.$store.state.Default.reduceButton },
      set (val) { this.$store.commit('TOGGLE_REDUCE_BUTTON', val) }
    },
    isVerticalNavMenuReduced () { return Boolean(this.reduce && this.reduceButton) },
    verticalNavMenuItemsMin ()  { return this.$store.state.Default.verticalNavMenuItemsMin },
    scrollbarTag ()             { return this.$store.getters.scrollbarTag          },
    windowWidth ()              { return this.$store.state.Default.windowWidth             }
  },
  watch: {
    '$route' () {
    ...
    reduce (val) {
      const verticalNavMenuWidth = val ? 'reduced' : this.$store.state.Default.windowWidth < 1200 ? 'no-nav-menu' : 'default'
